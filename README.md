...
- **npm audit: 12 moderate findings, `@opentelemetry/*` chain (source advisory `@opentelemetry/core` — "Unbounded memory allocation in W3C Baggage propagation", [GHSA-8988-4f7v-96qf](https://github.com/advisories/GHSA-8988-4f7v-96qf))** — transitive via `applicationinsights` (a production dependency added in Task 19 for server-side audit logging, `src/lib/logger.ts`). `applicationinsights@3.x` is built on OpenTelemetry and pulls in `@opentelemetry/core`, `@opentelemetry/exporter-{logs,metrics,trace}-otlp-http`, `@opentelemetry/exporter-metrics-otlp-proto`, `@opentelemetry/otlp-exporter-base`, `@opentelemetry/otlp-transformer`, `@opentelemetry/resources`, `@opentelemetry/sdk-logs`, `@opentelemetry/sdk-metrics`, and `@opentelemetry/sdk-trace-base` — all flagged moderate off the same advisory chain. `npm audit fix` (non-force) does not touch these; `npm audit fix --force` "resolves" them only by installing `applicationinsights@2.9.8`, a major-version *downgrade* from the `^3.15.1` this project uses (`isSemVerMajor: true` per the audit report), which predates `applicationinsights`'s move to OpenTelemetry and would require re-validating `src/lib/logger.ts` against a different API surface — not viable to take blindly. Tracked as an accepted exception; re-check `npm audit` whenever `applicationinsights` ships a release that updates its bundled OpenTelemetry dependencies past the vulnerable range.
- **npm audit: separately, 3 moderate findings in `prisma`'s dev-tooling chain (`@prisma/dev` → `@hono/node-server <1.19.13`, middleware bypass via repeated slashes, [GHSA-92pp-h63x-v22m](https://github.com/advisories/GHSA-92pp-h63x-v22m)) were fixed in this pass** via `npm audit fix` (non-force) — it bumped `prisma` from `7.8.0` to `7.9.0` (semver-minor, non-breaking) and dropped the vulnerable `@hono/node-server`. No further action needed; `npm audit` at last check reports **14** remaining moderate findings total (the two items above), zero low/high/critical.
