...
## Known Issues
As of this pass, `npm audit` reports **20 vulnerabilities (16 moderate, 4 high, 0 low/critical)** across four independent advisory chains. None are fixable via `npm audit fix` (non-force) — confirmed by running it for real: it makes zero changes to `package.json`/`package-lock.json` on the current tree. All four require either a version this project has already ruled out or a major-version downgrade of a direct dependency; each is tracked below as an accepted exception with its own re-check trigger.

- **2 moderate — `postcss <8.5.10`** (XSS via unescaped `</style>`, [GHSA-qx2v-qp2m-jg93](https://github.com/advisories/GHSA-qx2v-qp2m-jg93)). Transitive via `next`'s own bundled copy (`node_modules/next/node_modules/postcss`), not this project's code. `npm audit fix --force` only resolves it by downgrading `next` to `9.3.3`, which is not viable. Re-check `npm audit` whenever `next` is upgraded.
- **11 moderate — `@opentelemetry/*` chain** (source advisory `@opentelemetry/core` — "Unbounded memory allocation in W3C Baggage propagation", [GHSA-8988-4f7v-96qf](https://github.com/advisories/GHSA-8988-4f7v-96qf)). Transitive via `applicationinsights` (a production dependency added in Task 19 for server-side audit logging, `src/lib/logger.ts`). Affected packages: `@opentelemetry/core`, `@opentelemetry/exporter-{logs,metrics,trace}-otlp-http`, `@opentelemetry/exporter-metrics-otlp-proto`, `@opentelemetry/otlp-exporter-base`, `@opentelemetry/otlp-transformer`, `@opentelemetry/resources`, `@opentelemetry/sdk-logs`, `@opentelemetry/sdk-metrics`, `@opentelemetry/sdk-trace-base`. `npm audit fix --force` "resolves" this only by installing `applicationinsights@2.9.8`, a major-version *downgrade* from the `^3.15.1` this project uses (`isSemVerMajor: true` per the audit report) — 2.9.8 predates `applicationinsights`'s move to OpenTelemetry and would require re-validating `src/lib/logger.ts` against a different API surface, so it's not viable to take blindly. Re-check `npm audit` whenever `applicationinsights` ships a release that updates its bundled OpenTelemetry dependencies past the vulnerable range.
- **4 high — `@opentelemetry/propagator-jaeger` DoS** ("Denial of service in `JaegerPropagator` via unhandled exception on a malformed header", [GHSA-45rx-2jwx-cxfr](https://github.com/advisories/GHSA-45rx-2jwx-cxfr)). A separate advisory in the same `applicationinsights` → OpenTelemetry dependency tree as the item above, reached via `applicationinsights` → `@azure/monitor-opentelemetry` → `@opentelemetry/sdk-node` → `@opentelemetry/propagator-jaeger`. This chain's HIGH severity also bumps `applicationinsights`'s own rollup severity in `npm audit`'s report from moderate to **high**. Same root cause and same non-viable fix as above (`applicationinsights@2.9.8`, major downgrade) — re-check together with the item above whenever `applicationinsights` is upgraded.
- **3 moderate — `@hono/node-server` path traversal via `shadcn`** ("Path traversal in `serve-static` on Windows via encoded backslash (`%5C`)", [GHSA-frvp-7c67-39w9](https://github.com/advisories/GHSA-frvp-7c67-39w9)). Transitive via `shadcn` (a **direct** production dependency from the ShadCN UI setup in Task 1) → `@modelcontextprotocol/sdk` → `@hono/node-server`. This is a **different CVE** from the `@hono/node-server` finding fixed in an earlier pass (that one was `@prisma/dev`'s `GHSA-92pp-h63x-v22m`, a middleware-bypass advisory, resolved via a non-force `prisma` 7.8.0 → 7.9.0 bump and no longer present in `npm audit`'s output) — same package, two genuinely distinct advisories reached via two unrelated dependency paths. `npm audit fix --force` only resolves this one by downgrading `shadcn` to `3.8.3` (`isSemVerMajor: true`), a major-version downgrade of the installed `^4.13.0` that would require re-validating the whole ShadCN CLI/component setup — not viable to take blindly. Re-check `npm audit` whenever `shadcn` is upgraded.
